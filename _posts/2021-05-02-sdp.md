...
is in fact equivalent to \eqref{eq:sdp-lmi}, and we show how to establish this equivalence by appropriately constructing the decision variable \\(X\\), and the coefficients \\(C, A\_k\\), and \\(b_k\\). We leave the dimension of the new decision variable \\(X\\) vague on purpose for now, as the construction below will clarify it. Now, let us recall that the _trace_ of a square matrix is defined as the sum of its diagonal elements, and is connected to the standard Frobenius inner product on \\(\mathbb{R}^{m \times n}\\) via  
...
_Last updated: 2025-07-22 15:32 EST_